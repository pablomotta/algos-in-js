 ...
 * so that the cloned graph has the same vertices and edges as the original graph.
 * Let’s look at the below graphs as an example. If the input graph is G = (V, E)
 * where V is set of vertices and E is set of edges, then the output graph
 * (cloned graph) G’ = (V’, E’) such that V = V’ and E = E’.
 *
 *
